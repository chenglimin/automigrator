...
int SDPA_riscv::create_pipeline(const Option& _opt)
{
    ...
    if (scale != 0.f)
    {
        ...
        pd.set(3, 1);                   // transB (K: Seq x Embed -> K^T: Embed x Seq)
    ...
    // Attn * V
    {
        ...
        pd.set(3, 0);   // transB (V: Seq x Embed)
        pd.set(4, 0);   // constantA
        pd.set(5, 0);   // constantB
...
int SDPA_riscv::forward(const std::vector<Mat>& bottom_blobs, std::vector<Mat>& top_blobs, const Option& _opt) const
{
    ...
    for (int q = 0; q < num_heads; q++)
    {
        ...
        // Q * K^T with Gemm
        {
            ...
            bottom_blobs[0] = query_head;
            bottom_blobs[1] = key_head;
            std::vector<Mat> top_blobs2(1);
            top_blobs2[0] = qk_cross_head;
            int ret = _qk_gemm->forward(bottom_blobs, top_blobs2, opt);
            if (ret != 0)
            {
                ret_all = ret;
                goto cleanup;
        ...
        {
            int ret = qk_softmax->forward_inplace(qk_cross_head, opt);
            if (ret != 0)
            {
                ret_all = ret;
                goto cleanup;
    ...
    for (int q = 0; q < num_heads; q++)
    {
        ...
        std::vector<Mat> bottom_blobs(2);
        bottom_blobs[0] = qk_cross_head;
        bottom_blobs[1] = value_head;
        std::vector<Mat> top_blobs2(1);
        top_blobs2[0] = top_blob_head;
        int ret = qkv_gemm->forward(bottom_blobs, top_blobs2, opt);
        if (ret != 0)
        {
            ret_all = ret;
            goto cleanup;
...
cleanup:
    // destroy temp dynamic qk_gemm if created
    if (_qk_gemm_temp)
    {
        Option opt1 = opt;
